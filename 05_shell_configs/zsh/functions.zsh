...
function zfuncs() {
    if command -v gum &>/dev/null; then
        gum style --border double --border-foreground 57 --foreground 212 \
            --padding "1 2" --margin "1" "🥷 AVAILABLE DOJO CUSTOM SHELL FUNCTIONS"
    else
        print -P "\n%F{purple}🥷 AVAILABLE DOJO CUSTOM SHELL FUNCTIONS%f\n"
    fi
    print -P "  %F{cyan}🐙 git-magic%f      : Stage changes, commit, push and deploy in 1 step. Aliases: %F{yellow}gacp, gac, gpd%f"
    print -P "  %F{cyan}🛠️  create-web%f     : Web app scaffolding. Usage: %F{yellow}create-web vite my-project bun%f"
    print -P "  %F{cyan}⬇️  download%f       : Fast media downloading with yt-dlp. Usage: %F{yellow}download video <url>%f"
    print -P "  %F{cyan}🔐 verifyhash%f     : Integrity checksum calculator. Usage: %F{yellow}verifyhash calculate <file>%f"
    print -P "  %F{cyan}💻 vm%f             : Manage virtual machines using QEMU. Usage: %F{yellow}vm run 4G 2 disk.qcow2%f"
    print -P "  %F{cyan}📊 sysinfo%f        : System layout information. Usage: %F{yellow}sysinfo -a%f"
    print -P "  %F{cyan}📦 pkg / pkg-deb%f  : Unified package manager command wraps."
    print -P "\n  %F{yellow}💡 Tip: Type \`<command> -h\` or \`<command> --help\` to see detailed usage and flags.%f\n"
}
